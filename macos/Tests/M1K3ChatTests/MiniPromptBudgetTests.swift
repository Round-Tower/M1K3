...
//  Review: Kev + claude-opus-5, 2026-09-12, Confidence 0.85 — `includeExemplars:` became `exemplars: nil / .voice`.
...
struct MiniPromptBudgetTests {
    ...
    func personaFitsWithHeadroom() {
        ...
        let persona = M1K3Persona.systemPrompt(exemplars: nil)
    ...
    func exemplarsWouldFit() {
        ...
        let compact = Self.estimatedTokens(M1K3Persona.systemPrompt(exemplars: nil))
        let full = Self.estimatedTokens(M1K3Persona.systemPrompt(exemplars: .voice))
